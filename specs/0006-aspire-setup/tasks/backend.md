...
- [ ] B003 [US2] Create `HiveSpace.ServiceDefaults`
  ...
  - Preserve existing service-specific Serilog, MassTransit, EF, auth, IdentityServer, YARP, SignalR, and health behavior by adding defaults conservatively.
  - Do not make logs the only monitoring signal; traces, metrics, and health/status must be wired.
  - Acceptance: project builds and API projects can reference it without changing business behavior.
...
- [ ] B004 [US1] [US3] Create `HiveSpace.AppHost` infrastructure resources
  ...
  - Use resource names and connection-string names that map to `ConnectionStrings` keys: service database keys, `RabbitMq`, `Kafka`, `Redis`, `AzureServiceBus` when enabled, and `AzureStorage`.
  - Do not model production deployment settings or migrate existing Docker Compose volumes.
  - Acceptance: AppHost can instantiate all required infrastructure resources and exposes fixed local endpoints.
...
- [ ] B005 [US1] [US2] [US3] Create `AppHost API project resources`
  ...
  - Inject `Authentication__Authority`, gateway `ReverseProxy__Clusters__*__Destinations__destination1__Address`, service database connection strings, `ConnectionStrings__RabbitMq`, `ConnectionStrings__Kafka`, `ConnectionStrings__Redis`, `ConnectionStrings__AzureStorage`, and broker feature flags.
...
- [ ] B006 Update `MessagingOptions connection-string contract`
  ...
  - Move RabbitMQ host/port/username/password, Kafka bootstrap endpoint, and Azure Service Bus connection string reads to `ConnectionStrings:RabbitMq`, `ConnectionStrings:Kafka`, and `ConnectionStrings:AzureServiceBus`.
  - Keep only non-secret tuning under nested `Messaging` provider sections, such as RabbitMQ prefetch/outbox/heartbeat and Kafka client ID, consumer group, security protocol, and non-secret client behavior.
...
- [ ] B008 Update `messaging callers for new options`
  ...
  - Update every service that enables RabbitMQ, Kafka, or Azure Service Bus to pass connection-string-based values into shared messaging registration.
...
- [ ] B009 [US2] [US4] Update `ServiceDefaults references in API projects`
  ...
  - Acceptance: each included API project compiles with ServiceDefaults wired through the standardized startup path.
...
- [ ] B015 [US4] Update `OrderService startup structure`
  ...
  - Normalize startup file roles while preserving checkout saga, fulfillment saga, cart/order/coupon endpoint modules, EF setup, MassTransit, Kafka where enabled, and local URL `http://localhost:5004`.
...
- [ ] B019 [US1] Verify `MediaService Function v1 exclusion or safe launch`
  - File: `../hivespace.microservice/src/HiveSpace.MediaService/**`
  - Inspect the MediaService Function project and decide whether Aspire can launch it without behavior changes or extra runtime contract requirements.
  - If safe, add it as an AppHost resource and document the launch path; if not safe, keep it out of v1 and document verification-only status.
  - Do not change function processing behavior to force inclusion.
  - Acceptance: implementation summary states whether the function is included or explicitly out of v1 with no behavior change.
...
- [ ] B020 [US3] Update `backend local appsettings connection-string shape`
  ...
  - Move broker endpoints/secrets to `ConnectionStrings:RabbitMq`, `ConnectionStrings:Kafka`, `ConnectionStrings:AzureServiceBus`, and `ConnectionStrings:AzureStorage` as applicable.
