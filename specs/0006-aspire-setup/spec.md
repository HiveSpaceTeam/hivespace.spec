...
**Independent Test**: A developer can follow the documented startup flow from a clean local checkout and reach a running backend stack that includes the gateway, backend services, and required local dependencies.

**Acceptance Scenarios**:

1. **Given** a developer has the required local tooling installed, **When** they start the backend runtime using the documented flow, **Then** all required backend services and infrastructure dependencies begin startup from one entry point.
...
1. **Given** the local backend runtime is active, **When** the developer opens the runtime view, **Then** they can see every backend service and required dependency with a clear status.
...
As a HiveSpace developer, I want existing local URLs and ports to continue working so frontend apps and documented manual checks do not need a separate migration.
...
**Independent Test**: After starting the new local backend runtime, existing frontend apps and documented gateway URLs can still reach the backend through the same local addresses as before.

**Acceptance Scenarios**:

1. **Given** a frontend app is configured to call the existing local gateway URL, **When** the backend runtime is started through the new flow, **Then** the frontend app can still call the gateway without changing its local configuration.
...
**Why this priority**: The current backend has startup drift across `Program.cs`, `HostingExtensions.cs`, and `ServiceCollectionExtensions.cs`. Aspire integration should not add another cross-service pattern on top of inconsistent startup wiring.
...
1. **Given** an included API project, **When** a maintainer opens `Program.cs`, **Then** it acts as the entry point only and delegates service registration and pipeline setup to extension methods.
2. **Given** an included API project, **When** a maintainer opens `HostingExtensions.cs`, **Then** it orchestrates service registration and pipeline setup through `ConfigureServices()` and `ConfigurePipeline()` or `ConfigurePipelineAsync()` when async setup is required.
3. **Given** an included API project, **When** a maintainer opens `ServiceCollectionExtensions.cs`, **Then** `AddApp*()` methods are thin wrappers that delegate to shared helpers where available and contain only service-specific extras.
4. **Given** ApiGateway or a legacy-shaped service has service-specific startup behavior, **When** startup is standardized, **Then** existing routing, auth, session, CSRF, IdentityServer, controllers, SignalR, logging, and seeding behavior remains unchanged.
...
- A developer starts only part of the runtime and then starts the full runtime.
...
- **FR-006**: Existing local gateway and backend service addresses MUST remain usable after the new runtime flow is introduced.
...
- **FR-014**: Every included backend API project MUST follow the repository startup convention wherever technically possible: `Program.cs` as entry point only, `HostingExtensions.cs` for `ConfigureServices()` and pipeline setup, and `ServiceCollectionExtensions.cs` for thin `AddApp*()` service-registration wrappers.
- **FR-015**: ApiGateway MUST be brought into the same startup file structure by moving inline YARP, CORS, token validation, health check, and middleware wiring out of `Program.cs` while preserving existing gateway behavior.
...
- **FR-017**: Aspire ServiceDefaults and OpenTelemetry setup MUST be wired through the standardized startup path rather than scattered inline per service.
- **FR-018**: Infrastructure dependency endpoints and secrets MUST be configured through `ConnectionStrings`, including service databases, Redis, RabbitMQ, Kafka, Azure Service Bus, and Azure Storage/Azurite.
...
- **FR-021**: Non-secret broker tuning MAY remain under `Messaging` provider sections, such as RabbitMQ prefetch/outbox/heartbeat settings and Kafka client ID, consumer group, security protocol, and non-secret client behavior.
...
- **SC-002**: 100% of backend services listed in the architecture overview are represented in the local runtime workflow.
...
- **SC-008**: 100% of included API projects either follow the standard `Program.cs`/`HostingExtensions.cs`/`ServiceCollectionExtensions.cs` file roles or document a concrete technical reason why a specific startup step must remain different.
...
- The first version covers backend services and local infrastructure dependencies only; frontend dev servers remain outside this feature.
