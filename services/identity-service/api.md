...
| POST | `/api/v1/accounts/otp/request` | Anonymous | Request a one-time sign-in code for an email address; backed internally by the reusable `OtpChallenge` model with `Purpose = SignIn`; always returns `200 OK` with challenge token, expiry, and cooldown timestamp regardless of account existence, lock status, or cooldown state |
| POST | `/api/v1/accounts/otp/verify` | Anonymous | Verify OTP code using challenge token; only sign-in challenges are accepted in this feature; on success issues browser session cookies identical to password sign-in and returns a validated same-origin redirect URL |
