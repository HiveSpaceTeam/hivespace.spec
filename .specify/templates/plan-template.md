...
└── ADR-[NNNN]-[short-slug].md  # Conditional: required for non-obvious architecture decisions
```
...
Create these files during `/speckit-plan` only when the trigger condition is met.
...
- Number ADRs sequentially by checking existing files in `architecture/decisions/`.
- Use a short kebab-case slug in the file name.
- Use status `Draft` during planning; change to `Accepted` only when the decision is confirmed.
- Link the ADR from the plan's Summary or Technical Context when it affects implementation tasks.
